...
class ItemControllerTest {
    ...
    void getAll() throws Exception {
        when(mapper.fromItemToDto(any(Item.class)))
                .thenReturn(itemDto);
        when(mapper.fromBookingToDto(any()))
                .thenReturn(new BookingDto());
        when(mapper.fromCommentToDto(any(Comment.class)))
    ...
    void patchItem() throws Exception {
        when(mapper.fromItemToDto(any()))
                .thenReturn(itemDto);
        when(itemService.patchItem(anyLong(), any(Item.class), anyLong()))
                .thenReturn(item2);
        ...
        verify(itemService, times(1)).patchItem(anyLong(), eq(null), anyLong());
        verify(mapper, times(1)).fromItemToDto(any());
        verifyNoMoreInteractions(itemService);
    }
    ...
    void searchItem() throws Exception {
        ...
        verify(itemService, times(1)).searchItem(anyString(), anyLong(), anyInt(), anyInt());
        verifyNoMoreInteractions(itemService);
    }
    ...
    void postComment() throws Exception {
        ...
        verify(itemService, times(1)).postComment(anyLong(), anyLong(), any());
        verifyNoMoreInteractions(itemService);
    }
}
